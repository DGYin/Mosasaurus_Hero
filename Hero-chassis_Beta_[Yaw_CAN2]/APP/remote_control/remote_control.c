/**
  ...
  * @version V1.0.0
  * @date    12.2021
  * @brief   该文件是接收遥控器对应的通道值和根据遥控器来判断对应的模式（当遥控器接收器插在底盘上时）
  ******************************************************************************
  * @attention
  ******************************************************************************
...
void remote_control(void)//遥控器通道值转化以及模式选择
{
		...
		gimbal_y.Target_Angle = gimbal_y.Target_Angle + (rc_ctrl.rc.ch[0]-1024)/200.00f;
	switch(rc_ctrl.rc.s[1])                                //S1（左拨杆）
	{
	...
	switch(rc_ctrl.rc.s[0])                                //S1（左拨杆）
	{
		case 3: {shoot_flag=1;} break;//正常模式
		case 1: {shoot_flag=0;} break;//底盘锁死模式
		case 2: {chassis_control_order.chassis_mode=CHASSIS_SPIN;} break;//小陀螺模式
		default:break;
	}
}
